...
def mnist_sum_4_loader(data_dir, batch_size):
...
class MNISTSum4Net(nn.Module):
  def __init__(self):
    ...
    self.bbox = blackbox.BlackBox(
      ...
      (blackbox.DiscreteInputMapping(list(range(10))), 
       ...
       blackbox.DiscreteInputMapping(list(range(10))),
       blackbox.DiscreteInputMapping(list(range(10)))),
      blackbox.DiscreteOutputMapping(list(range(37)))
    )
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor]):
    ...
    return self.bbox(a_distrs, b_distrs, c_distrs, d_distrs) # Tensor 64 x 19


class Trainer():
  def __init__(self, train_loader, test_loader, model_dir, learning_rate):
    self.model_dir = model_dir
    self.network = MNISTSum4Net()
...
if __name__ == "__main__":
  ...
  # Dataloaders
  train_loader, test_loader = mnist_sum_4_loader(data_dir, batch_size)
