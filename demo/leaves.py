...
class LeavesNet(nn.Module):
  def __init__(self, sample_count):
    ...
    self.margin = ["entire", "serrate", "lobbed", "mixed", "dentated"]
    self.shape = ["cordate", "heart", "circle", "elliptical", "linear", "other"]
    self.apex = ["aristate", "round", "half"]
    self.labels = ["apple", "blueberry", "cherry", "grape", "peach", "pepper", "potato", "raspberry", "soybean", "strawberry", "tomato"]
    ...
    self.apex_fc = nn.Sequential(
      nn.Linear(12544, 256),
      nn.ReLU(),
      nn.Linear(256, 64),
      nn.ReLU(),
      nn.Linear(64, len(self.apex))
    )

    # Blackbox encoding identification chart
    self.bbox = blackbox.BlackBox(
      task_program.classify_plantvillage,
      (blackbox.DiscreteInputMapping(self.margin),
       blackbox.DiscreteInputMapping(self.shape),
       blackbox.DiscreteInputMapping(self.apex)),
      blackbox.DiscreteOutputMapping(self.labels))

  def forward(self, x):
    x = self.cnn(x)
    has_margin = F.softmax(self.margin_fc(x), dim=1)
    has_shape = F.softmax(self.shape_fc(x), dim=1)
    has_apex = F.softmax(self.apex_fc(x), dim=1)
    return self.bbox(has_margin, has_shape, has_apex)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--data-dir", type=str, default="leaf_plantvillage")
