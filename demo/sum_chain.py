...
class MNISTSumChainNet(nn.Module):
  def __init__(self):
    super(MNISTSumChainNet, self).__init__()

    # MNIST Digit Recognition Network
    self.mnist_net = MNISTNet()
    self.bbox1 = blackbox.BlackBox(
      task_program.sum_2,
      (blackbox.DiscreteInputMapping(list(range(10))), 
       blackbox.DiscreteInputMapping(list(range(10)))),
      blackbox.DiscreteOutputMapping(list(range(19)))
    )
    self.bbox2 = blackbox.BlackBox(
      task_program.sum_2,
      (blackbox.DiscreteInputMapping(list(range(19))), 
       blackbox.DiscreteInputMapping(list(range(10)))),
      blackbox.DiscreteOutputMapping(list(range(28)))
    )
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor]):
    ...
    output1 = self.bbox1(a_distrs, b_distrs)
    output2 = self.bbox2(output1, c_distrs)
    return output2

class Trainer():
  def __init__(self, train_loader, test_loader, model_dir, learning_rate):
    self.model_dir = model_dir
    self.network = MNISTSumChainNet()
...
if __name__ == "__main__":
  # Argument parser
  parser = ArgumentParser("mnist_sum_chain")
  ...
  model_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../model/mnist_sum_chain"))
