...
from torch_modules.ised import ISED
from src.output import DiscreteOutputMapping
from src.input import DiscreteInputMapping, SingleInput
...
class MNISTSumNet(nn.Module):
  def __init__(self, digit):
    ...
    self.bbox = ISED(**{
        ...
        "output_mapping": DiscreteOutputMapping(list(range(self.digit*9+1)), loss_aggregator="add-mult"),
        "n_samples": 100,
        "semiring": "add-mult"
        })
  ...
  def forward(self, x):
    ...
    x = [SingleInput(x[i]) for i in range(self.digit)]
