# With pretraining: up to 94% accuracy for 50 epochs
...
class TensorSketch:
    def __init__(self, num_inputs, input_size, output_size, rank=3):
        """
        Initialize a tensor sketch
        :param num_inputs: The number of inputs to the black-box program
        :param input_size: The number of classes for each input (for MNIST, this is 10)
        :param output_size: The number of possible outputs
        :param rank: The rank of the sketch
        """
        self.rank = rank
        self.num_inputs = num_inputs
        self.input_size = input_size
        self.output_size = output_size
        self.V = torch.randn(10, 10, output_size, requires_grad=True)

    def forward(self, *logits):
        """
        Forward pass to compute the tensor sketch.
        :param logits1: Logits for the first digit (batch_size x input_size)
        :param logits2: Logits for the second digit (batch_size x input_size)
        :return: Approximate sum distribution (batch_size x output_size)
        """
        batch_size = logits[0].shape[0]
        outer_prod = torch.stack([torch.outer(logits[0][i], logits[1][i]) for i in range(batch_size)])
        sketch_output = torch.einsum('bij,jik->bk', outer_prod, self.V)
        return F.softmax(sketch_output, dim=-1)
...
class MNISTRNet(nn.Module):
  def __init__(self, k, semiring, digit, min_output, max_output, bbox):
    super(MNISTRNet, self).__init__()

    self.digit = digit
    # MNIST Digit Recognition Network
    self.mnist_net = MNISTNet()
    # state_dict = torch.load('/Users/alaiasolko-breslin/Penn/research/blackbox-learning/model/mnist_r/sum_2_best.pt')
    # new_state_dict = {}
    # for key in state_dict.keys():
    #     new_key = key.replace('mnist_net.', '')  # Remove 'net.' from the key
    #     new_state_dict[new_key] = state_dict[key]
    # self.mnist_net.load_state_dict(new_state_dict)
    # self.mnist_net.eval()
    self.tensor_sketch = TensorSketch(num_inputs=2, input_size=10, output_size=18)
  ...
  def forward(self, x):
    ...
    return self.tensor_sketch.forward(*tuple(x))
  
  def parameters(self):
    return self.tensor_sketch.parameters()
...
class Trainer():
  def __init__(self,
    ...
    self.network = MNISTRNet(k, semiring, digit, min_output, max_output, bbox)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--max-output", type=int, default=18)
  ...
  trainer = Trainer(train_loader, test_loader, model_dir, learning_rate, k, semiring, digit, min_output, max_output, bbox)
