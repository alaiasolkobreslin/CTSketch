...
class CLUTRRModel(nn.Module):
  def __init__(
    ...
    self.device = device
    self.debug = debug
    self.no_fine_tune_deberta = no_fine_tune_deberta
    self.scallop_softmax = scallop_softmax

    # Deberta as embedding extraction model
    self.tokenizer = DebertaV2Tokenizer.from_pretrained("microsoft/deberta-v3-base")
    self.deberta_model = DebertaV2Model.from_pretrained("microsoft/deberta-v3-base")
    self.embed_dim = self.deberta_model.config.hidden_size
  ...
  def _extract_relations(self, clean_contexts, clean_context_splits, name_token_indices_maps):
    ...
    encoded_contexts = self.deberta_model(context_input_ids, context_attention_mask)
    if self.no_fine_tune_deberta:
      deberta_embedding = encoded_contexts.last_hidden_state.detach()
    else:
      deberta_embedding = encoded_contexts.last_hidden_state
    ...
    for (begin, end) in clean_context_splits:
      ...
      for (j, name_token_indices) in zip(range(begin, end), name_token_indices_maps[begin:end]):
        ...
        feature_maps = {}
        curr_sentence_rep.append(torch.mean(deberta_embedding[j, :sum(context_attention_mask[j]), :], dim=0))
        for (name, token_indices) in name_token_indices.items():
          token_features = deberta_embedding[j, token_indices, :]
...
class Trainer:
  def __init__(self, train_loader, test_loader, device, model_dir, model_name, learning_rate, **args):
    ...
    if load_model:
      ...
      new_model.deberta_model = loaded_model.deberta_model
...
if __name__ == "__main__":
  ...
  parser.add_argument("--model-name", type=str, default="clutrr_global_deberta_1")
  ...
  parser.add_argument("--no-fine-tune-deberta", type=bool, default=False)
