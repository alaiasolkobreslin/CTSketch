...
class CLUTRRModel(nn.Module):
  def __init__(self, device="cpu", num_mlp_layers=1, debug=False, no_fine_tune_roberta=False, use_softmax=False, provenance="difftopbottomkclauses", train_top_k=3, test_top_k=3, scallop_softmax=False, w2v_model=None):
    ...
    # word2vec as embedding extraction model
    self.embed_dim = w2v_model.vector_size
    self.lstm = nn.LSTM(self.embed_dim, self.embed_dim, bidirectional=True, batch_first=True)

    # Entity embedding
    self.relation_extraction = MLP(self.embed_dim * 6, self.embed_dim, len(relation_id_map), num_layers=num_mlp_layers, sigmoid=not use_softmax, softmax=use_softmax)
  ...
  def _preprocess_contexts(self, contexts, context_splits):
    ...
    for (_, (start, end)) in enumerate(context_splits):
      ...
      for (j, sentence) in zip(range(start, end), contexts[start:end]):
        ...
        current_token_id = 0
        for phrase_id, tokens in enumerate([list(gensim.utils.tokenize(phrase)) for phrase in splitted]):
            ...
            sentence_token.append(token_ls)
            sentence_ids.append(token_ids)
            if phrase_id in [i for ids in is_name_ids.values() for i in ids]:
              assert(len(token_ids) > 0)

        name_token_indices = {s: [k for phrase_id in is_name_ids[s] for k in sentence_ids[phrase_id]] for s in names}
        ...
        for tokens in sentence_token:
          for token in tokens:
            if token in w2v_model.key_to_index:
              ...
              word_embeddings.append(np.zeros(self.embed_dim, dtype="float32"))

        all_word_embeddings.append(torch.tensor(word_embeddings, device = self.device))

        # Clean up the sentence and add it to the batch
        clean_sentence = [t for tls in sentence_token for t in tls]
    ...
    return (all_word_embeddings, clean_context_splits, name_token_indices_maps)

  def _extract_relations(self, word_embeddings, clean_context_splits, name_token_indices_maps):
    ...
    for (begin, end) in clean_context_splits:
      ...
      for (j, name_token_indices) in zip(range(begin, end), name_token_indices_maps[begin:end]):
        ...
        if len(word_embeddings[j].shape) == 1:
          continue

        lstm_encoded_contexts, (h, _) = self.lstm(word_embeddings[j].reshape(1, len(word_embeddings[j]), -1))
        curr_sentence_rep.append(h.reshape(-1))
        for (name, token_indices) in name_token_indices.items():
          token_features = lstm_encoded_contexts[0, token_indices, :]
...
if __name__ == "__main__":
  parser = ArgumentParser()
   ...
   # parser.add_argument("--model-name", type=str, default="clutrr_embed_2")
   parser.add_argument("--model-name", type=str, default="clutrr_ablation_w2v_bilstm")
   ...
   w2v_model = KeyedVectors.load_word2vec_format(os.path.join(data_root, 'CLUTRR/GoogleNews-vectors-negative300.bin'), binary=True)

   # Train
   trainer = Trainer(train_loader, test_loader, device, model_dir, args.model_name, args.learning_rate, num_mlp_layers=args.num_mlp_layers, debug=args.debug, provenance=args.provenance, train_top_k=args.train_top_k, test_top_k=args.test_top_k, use_softmax=args.use_softmax, no_fine_tune_roberta=args.no_fine_tune_roberta, scallop_softmax = args.scallop_softmax,load_model=args.load_model, w2v_model = w2v_model)
