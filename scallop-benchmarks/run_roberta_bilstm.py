...
class CLUTRRModel(nn.Module):
  def __init__(self, device="cpu", num_mlp_layers=1, debug=False, no_fine_tune_roberta=False, use_softmax=False, provenance="difftopbottomkclauses", top_k=3):
    ...
    # Entity embedding
    self.relation_extraction = MLP(self.embed_dim * 4, self.embed_dim, len(relation_id_map), num_layers=num_mlp_layers, sigmoid=not use_softmax, softmax=use_softmax)
  ...
  def _extract_relations(self, clean_contexts, clean_context_splits, name_token_indices_maps):
    ...
    if self.no_fine_tune_roberta:
        hidden_state = encoded_contexts.last_hidden_state.detach()
    else:
        hidden_state = encoded_contexts.last_hidden_state
    lstm_encoded_contexts, _ = self.lstm(hidden_state)
    ...
    for (begin, end) in clean_context_splits:
      ...
      for (j, name_token_indices) in zip(range(begin, end), name_token_indices_maps[begin:end]):
        ...
        for (name, token_indices) in name_token_indices.items():
          token_features = lstm_encoded_contexts[j, token_indices, :]
...
if __name__ == "__main__":
  ...
  # Train
  trainer = Trainer(train_loader, test_loader, device, model_dir, args.model_name, args.learning_rate, num_mlp_layers=args.num_mlp_layers, debug=args.debug, provenance=args.provenance, top_k=args.top_k, use_softmax=args.use_softmax, no_fine_tune_roberta=args.no_fine_tune_roberta, load_model=args.load_model)
