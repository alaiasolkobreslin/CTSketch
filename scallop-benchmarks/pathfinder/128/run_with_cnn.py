...
class PathFinder128Net(nn.Module):
  def __init__(
    ...
    self.ctx.add_relation("edge", ("i8", "i8"), input_mapping=self.adjacency)
    ...
    self.ctx.add_rule("endpoints_connected() = is_endpoint(x), is_endpoint(y), path(x, y), x != y")
    self.connected = self.ctx.forward_function("endpoints_connected", ())
