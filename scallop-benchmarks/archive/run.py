...
class CLUTRRModel(nn.Module):
  ...
  def forward(self, x):
    ...
    encoded_sentence = self.roberta_model(input_ids, attention_mask)
    if self.use_last_hidden_state:
      sentence_embeddings = encoded_sentence.last_hidden_state[:, -1, :]
    else:
    ...
    for (_, (start, end)) in enumerate(context_splits):
      ...
      for (j, sentence) in zip(range(start, end), contexts[start:end]):
        ...
        name_input_ids = name_tokens["input_ids"].to(self.device)
        name_attention_mask = name_tokens["attention_mask"].to(self.device)
        if self.use_last_hidden_state:
          name_embeddings = self.roberta_model(name_input_ids, name_attention_mask).last_hidden_state[:, -1, :]
        else:
          name_embeddings = self.roberta_model(name_input_ids, name_attention_mask).pooler_output
...
if __name__ == "__main__":
  ...
  parser.add_argument("--num-mlp-layers", type=int, default=0)
