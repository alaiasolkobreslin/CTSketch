...
objects_long = bathroom_objects + bedroom_objects + office_objects + lab_objects + lobby_objects \
          + basement_objects + dining_objects + kitchen_objects + living_objects
...
class Trainer():
  def __init__(self, model, train_loader, test_loader, learning_rate, model_dir, seed):
  ...
  def train_epoch(self, epoch):
    ...
    for (input, file, target) in self.train_loader:
      ...
      _, output = self.network(input.to(self.device), cls.to(self.device), box_len.to(self.device), conf.to(self.device))
  ...
  def test_epoch(self, epoch):
    ...
    if self.best_loss is None or test_loss < self.best_loss:
      ...
      torch.save(self.network.state_dict(), self.model_dir+f"/{self.seed}_best.pth")
    
    return float(num_correct/num_items)

  def train(self, n_epochs):
    dict = {}
    # self.network.load_state_dict(torch.load(self.model_dir+f"/{self.seed}_last.pth"))
    ...
    torch.save(self.network.state_dict(), self.model_dir+f"/{self.seed}_last.pth")
    return dict